#include <termios.h>
#include <unistd.h>
#include <fcntl.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <sys/ioctl.h>
#include <signal.h>

#include "Cimpl_gcurses.h"
#include "objects.h"

char GCS_VAR_COLOR_BLACK[]          = "\x1b[38;5;0m";
char GCS_VAR_COLOR_RED[]            = "\x1b[38;5;1m";
char GCS_VAR_COLOR_GREEN[]          = "\x1b[38;5;2m";
char GCS_VAR_COLOR_YELLOW[]         = "\x1b[38;5;3m";
char GCS_VAR_COLOR_BLUE[]           = "\x1b[38;5;4m";
char GCS_VAR_COLOR_MAGENTA[]        = "\x1b[38;5;5m";
char GCS_VAR_COLOR_CYAN[]           = "\x1b[38;5;6m";
char GCS_VAR_COLOR_WHITE[]          = "\x1b[38;5;7m";

char GCS_VAR_COLOR_BRIGHT_BLACK[]   = "\x1b[38;5;8m";
char GCS_VAR_COLOR_BRIGHT_RED[]     = "\x1b[38;5;9m";
char GCS_VAR_COLOR_BRIGHT_GREEN[]   = "\x1b[38;5;10m";
char GCS_VAR_COLOR_BRIGHT_YELLOW[]  = "\x1b[38;5;11m";
char GCS_VAR_COLOR_BRIGHT_BLUE[]    = "\x1b[38;5;12m";
char GCS_VAR_COLOR_BRIGHT_MAGENTA[] = "\x1b[38;5;13m";
char GCS_VAR_COLOR_BRIGHT_CYAN[]    = "\x1b[38;5;14m";
char GCS_VAR_COLOR_BRIGHT_WHITE[]   = "\x1b[38;5;15m";

char GCS_VAR_COLOR_DEFAULT[]        = "\x1b[39m";
char GCS_VAR_COLOR_RESET[]          = "\x1b[0m";

char GCS_VAR_BG_COLOR_BLACK[]          = "\x1b[48;5;0m";
char GCS_VAR_BG_COLOR_RED[]            = "\x1b[48;5;1m";
char GCS_VAR_BG_COLOR_GREEN[]          = "\x1b[48;5;2m";
char GCS_VAR_BG_COLOR_YELLOW[]         = "\x1b[48;5;3m";
char GCS_VAR_BG_COLOR_BLUE[]           = "\x1b[48;5;4m";
char GCS_VAR_BG_COLOR_MAGENTA[]        = "\x1b[48;5;5m";
char GCS_VAR_BG_COLOR_CYAN[]           = "\x1b[48;5;6m";
char GCS_VAR_BG_COLOR_WHITE[]          = "\x1b[48;5;7m";

char GCS_VAR_BG_COLOR_BRIGHT_BLACK[]   = "\x1b[48;5;8m";
char GCS_VAR_BG_COLOR_BRIGHT_RED[]     = "\x1b[48;5;9m";
char GCS_VAR_BG_COLOR_BRIGHT_GREEN[]   = "\x1b[48;5;10m";
char GCS_VAR_BG_COLOR_BRIGHT_YELLOW[]  = "\x1b[48;5;11m";
char GCS_VAR_BG_COLOR_BRIGHT_BLUE[]    = "\x1b[48;5;12m";
char GCS_VAR_BG_COLOR_BRIGHT_MAGENTA[] = "\x1b[48;5;13m";
char GCS_VAR_BG_COLOR_BRIGHT_CYAN[]    = "\x1b[48;5;14m";
char GCS_VAR_BG_COLOR_BRIGHT_WHITE[]   = "\x1b[48;5;15m";

char GCS_VAR_BG_COLOR_DEFAULT[]        = "\x1b[49m";
char GCS_VAR_BG_COLOR_RESET[]          = "\x1b[0m";

term_w_t *original_term;

/***** DEPRICATED *****/
static void sigint_handler(int sig){
    (void)sig;                     /* unused */

    term_w_t *terminal = original_term;
    if(terminal){
        tcsetattr(STDIN_FILENO, TCSANOW, &terminal->old_conf);   /* restore term */
    }

    write(STDOUT_FILENO, "\nCtrl-C caught - restoring terminal and exiting.\n", 27);
    signal(SIGINT, SIG_DFL);       /* back to default */
    raise(SIGINT);                 /* now exit with the usual code */
}
/**********************/

/***** DEPRICATED *****/
static void segv_handler(int sig, siginfo_t *info, void *ctx) {
    (void)sig; (void)info; (void)ctx;

    term_w_t *terminal = original_term;
    if(terminal){
        tcsetattr(STDIN_FILENO, TCSANOW, &terminal->old_conf);   /* restore term */       
    }

    write(STDOUT_FILENO, "\n*** SEGFAULT ***\n", 18);
    signal(SIGSEGV, SIG_DFL);   /* default action */
    raise(SIGSEGV);
}
/**********************/

/***** DEPRICATED *****/
void move_cursor(term_w_t *terminal, int r, int c){
    printf("\x1b[%d;%dH",r+1,c+1);
    terminal->cursor_data.row = r;
    terminal->cursor_data.col = c;
}
/**********************/


/**** IMPLEMENTED *****/
/*
    1/true to set non-blocking
    0/false to set blocking
*/
def_method(io_blocking_handler, int)
int io_blocking_handler(term_w_t *terminal, int setting){

    if(setting){
        tcsetattr(STDIN_FILENO, TCSANOW, &terminal->old_conf);
        int flags = fcntl(STDIN_FILENO, F_GETFL, 0);
        if (flags != -1) fcntl(STDIN_FILENO, F_SETFL, flags & ~O_NONBLOCK);
        setvbuf(stdout, NULL, _IOLBF, 0); 
    }else{
        if (tcgetattr(STDIN_FILENO, &terminal->old_conf) == -1) return -1;

        terminal->new_conf = terminal->old_conf;
        terminal->new_conf.c_lflag &= ~(ICANON | ECHO);
        terminal->new_conf.c_cc[VMIN]  = 0;
        terminal->new_conf.c_cc[VTIME] = 0;

        if (tcsetattr(STDIN_FILENO, TCSANOW, &terminal->new_conf) == -1) return -1;

        int flags = fcntl(STDIN_FILENO, F_GETFL, 0);
        if (flags == -1) return -1;

        setvbuf(stdout, NULL, _IONBF, 0);

        return fcntl(STDIN_FILENO, F_SETFL, flags | O_NONBLOCK);        
    }

}
/**********************/

/**** IMPLEMENTED *****/
def_action(clear_screen);
void clear_screen(term_w_t *terminal){

    //puts("\x1b[2J");

    if(!terminal->cursor_data.hidden) hide_cursor();

    for(int i=0;i<terminal->ncols*terminal->nrows;i++){
            strcpy(terminal->term_frame[i].symbol, " ");
            terminal->term_frame[i].color = GCS_DEFAULT;
            terminal->term_frame[i].bg_color = GCS_BG_DEFAULT;
    }

    if(!terminal->cursor_data.hidden) show_cursor();
}
/**********************/

/**** IMPLEMENTED *****/
def_action(present)
void present(term_w_t *terminal){

    if(!terminal->cursor_data.hidden) hide_cursor();
    cursor_origin();

    for(int i=0; i<terminal->nrows*terminal->ncols; i++){
        if(strcmp(terminal->term_frame[i].symbol, terminal->prev_frame[i].symbol)||
           strcmp(terminal->term_frame[i].color,terminal->prev_frame[i].color)||
           strcmp(terminal->term_frame[i].bg_color,terminal->prev_frame[i].bg_color)){
            printf("\x1b[%d;%dH", i/terminal->ncols+1, i%terminal->ncols+1);
            printf("%s%s%s\x1b[0m",terminal->term_frame[i].bg_color, terminal->term_frame[i].color, terminal->term_frame[i].symbol);
        }
    }

    printf("\x1b[%d;%dH", terminal->cursor_data.row+1, terminal->cursor_data.col+1);
    if(!terminal->cursor_data.hidden) show_cursor();

    fflush(stdout);
    memcpy(terminal->prev_frame, terminal->term_frame, sizeof(tile_t)*terminal->ncols*terminal->nrows);
}
/**********************/

/**** IMPLEMENTED *****/
def_method(cursor_handler, int)
void cursor_handler(term_w_t *terminal, int status){

    if(status){
        show_cursor();
        terminal->cursor_data.hidden = 0;
    }else{
        hide_cursor();
        terminal->cursor_data.hidden = 1;
    }
}
/**********************/

/**** IMPLEMENTED *****/
def_method(horz_strdisp, str_t);
int horz_strdisp(term_w_t *terminal, str_t string){
    int r = string.r;
    int c = string.c;
    char *source = string.source;

    int i=0;
    while(source[i]!=0){
        if(r>=0&&r<terminal->nrows && c>=0&&c<terminal->ncols){
            (terminal->term_frame+r*terminal->ncols + c)->symbol[0] = source[i];
            (terminal->term_frame+r*terminal->ncols + c)->symbol[1] = 0;
            (terminal->term_frame+r*terminal->ncols + c)->color = GCS_DEFAULT;
            (terminal->term_frame+r*terminal->ncols + c)->bg_color = GCS_BG_DEFAULT;
        }
        i++;
        c++;
    }
    return i;
}
/**********************/

/**** IMPLEMENTED *****/
def_method(vert_strdisp, str_t)
int vert_strdisp(term_w_t *terminal, str_t string){
    int r = string.r;
    int c = string.c;
    char *source = string.source;

    int i=0;
    while(source[i]!=0){
        if(r>=0&&r<terminal->nrows && c>=0&&c<terminal->ncols){
            (terminal->term_frame+r*terminal->ncols + c)->symbol[0] = source[i];
            (terminal->term_frame+r*terminal->ncols + c)->symbol[1] = 0;
            (terminal->term_frame+r*terminal->ncols + c)->color = GCS_DEFAULT;
            (terminal->term_frame+r*terminal->ncols + c)->bg_color = GCS_BG_DEFAULT;
        }
        i++;
        r++;
    }
    return i;    
}
/**********************/

/**** IMPLEMENTED *****/
def_method(horz_tiledisp, fstr_t)
int horz_tiledisp(term_w_t *terminal, fstr_t tile){
    int r = tile.r;
    int c = tile.c;
    tile_t *source = tile.source;

    int i=0;
    while(source[i].symbol[0]!=0){
        if(r>=0&&r<terminal->nrows && c>=0&&c<terminal->ncols){
            strcpy((terminal->term_frame+r*terminal->ncols + c)->symbol, source[i].symbol);
            (terminal->term_frame+r*terminal->ncols + c)->color = source[i].color;
            (terminal->term_frame+r*terminal->ncols + c)->bg_color = source[i].bg_color;
        }
        i++;
        c++;
    }
    return i;
}
/**********************/

/**** IMPLEMENTED *****/
def_method(vert_tiledisp, fstr_t)
int vert_tiledisp(term_w_t *terminal, fstr_t tile){
    int r = tile.r;
    int c = tile.c;
    tile_t *source = tile.source;

    int i=0;
    while(source[i].symbol[0]!=0){
        if(r>=0&&r<terminal->nrows && c>=0&&c<terminal->ncols){
            strcpy((terminal->term_frame+r*terminal->ncols + c)->symbol, source[i].symbol);
            (terminal->term_frame+r*terminal->ncols + c)->color = source[i].color;
            (terminal->term_frame+r*terminal->ncols + c)->bg_color = source[i].bg_color;
        }
        i++;
        r++;
    }
    return i;
}
/**********************/

/**** IMPLEMENTED *****/
def_method(draw_rect, rect_t)
void draw_rect(term_w_t *terminal, rect_t rectangle){
    int i,j;
    for(i=rectangle.r;i<rectangle.h+rectangle.r;i++){
        for(j=rectangle.c;j<rectangle.w+rectangle.c;j++){
            strcpy((terminal->term_frame + (i*terminal->ncols) + j)->symbol, rectangle.tile.symbol);
            (terminal->term_frame + (i*terminal->ncols) + j)->color = rectangle.tile.color;
            (terminal->term_frame + (i*terminal->ncols) + j)->bg_color = rectangle.tile.bg_color;
        }
    }
}
/**********************/

/**** IMPLEMENTED *****/
def_method(draw_border, rect_t)
void draw_border(term_w_t *terminal, rect_t rectangle){

    int i,j;
    for(i=rectangle.r;i<rectangle.h+rectangle.r;i++){
        for(j=rectangle.c;j<rectangle.w+rectangle.c;j++){
            
            if(i==rectangle.r||i==rectangle.h+rectangle.r-1){
                strcpy((terminal->term_frame + (i*terminal->ncols) + j)->symbol, rectangle.tile.symbol);
                (terminal->term_frame + (i*terminal->ncols) + j)->color = rectangle.tile.color;
                (terminal->term_frame + (i*terminal->ncols) + j)->bg_color = rectangle.tile.bg_color;
            }
            if(j==rectangle.c||j==rectangle.w+rectangle.c-1){
                strcpy((terminal->term_frame + (i*terminal->ncols) + j)->symbol , rectangle.tile.symbol);
                (terminal->term_frame + (i*terminal->ncols) + j)->color = rectangle.tile.color;
                (terminal->term_frame + (i*terminal->ncols) + j)->bg_color = rectangle.tile.bg_color;
            }
        
        }
    }
}
/**********************/

/***** DEPRICATED *****/
//helper for draw_frame
void set_frame_tile(term_w_t *terminal, int r, int c,
                    const char *symbol, char *color, char *bg_color){
    if (r<0 || r>=terminal->nrows ||
        c<0 || c>=terminal->ncols)
        return;

    tile_t *tile = &terminal->term_frame[r * terminal->ncols + c];

    strcpy(tile->symbol, symbol);
    tile->color = color;
    tile->bg_color = bg_color;
}
/**********************/

/**** IMPLEMENTED *****/
def_method(draw_frame, rect_t)
void draw_frame(term_w_t *terminal, rect_t rectangle){

    if (!terminal || !terminal->term_frame) return;
    if (rectangle.w < 2 || rectangle.h < 2) return;

    int i;
    
    set_frame_tile(terminal, rectangle.r, rectangle.c,
                   "┌", rectangle.tile.color, rectangle.tile.bg_color);

    set_frame_tile(terminal, rectangle.r,
                   rectangle.c + rectangle.w - 1,
                   "┐", rectangle.tile.color, rectangle.tile.bg_color);

    set_frame_tile(terminal,
                   rectangle.r + rectangle.h - 1,
                   rectangle.c,
                   "└", rectangle.tile.color, rectangle.tile.bg_color);

    set_frame_tile(terminal,
                   rectangle.r + rectangle.h - 1,
                   rectangle.c + rectangle.w - 1,
                   "┘", rectangle.tile.color, rectangle.tile.bg_color);

    for(i=1;i<rectangle.w-1;i++){
        set_frame_tile(terminal,
                       rectangle.r,
                       rectangle.c + i,
                       "─", rectangle.tile.color, rectangle.tile.bg_color);

        set_frame_tile(terminal,
                       rectangle.r + rectangle.h - 1,
                       rectangle.c + i,
                       "─", rectangle.tile.color, rectangle.tile.bg_color);
    }
    for(i=1;i<rectangle.h-1;i++){
        set_frame_tile(terminal,
                       rectangle.r + i,
                       rectangle.c,
                       "│", rectangle.tile.color, rectangle.tile.bg_color);

        set_frame_tile(terminal,
                       rectangle.r + i,
                       rectangle.c + rectangle.w - 1,
                       "│", rectangle.tile.color, rectangle.tile.bg_color);
    }

}
/**********************/

/**** IMPLEMENTED *****/
int detect_resize(term_w_t *terminal){
    struct winsize ws;
    ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws);
    if(terminal->nrows!=ws.ws_row||terminal->ncols!=ws.ws_col){
        return 1;
    }else{
        return 0;
    }
}
/**********************/

/**** IMPLEMENTED *****/
def_action(frame_resize)
void frame_resize(term_w_t *terminal){

    if(detect_resize(terminal)){
        struct winsize ws;

        while(detect_resize(terminal)){
            puts("\x1b[2J");
            ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws);  
            terminal->nrows = ws.ws_row;
            terminal->ncols = ws.ws_col;
            usleep(125000);
        }
        


        free(terminal->term_frame);
        free(terminal->prev_frame);
        terminal->term_frame = malloc(sizeof(tile_t)*terminal->nrows*terminal->ncols);
        terminal->prev_frame = malloc(sizeof(tile_t)*terminal->nrows*terminal->ncols);
        int i;
        for(i=0;i<terminal->nrows*terminal->ncols;i++){
            strcpy(terminal->term_frame[i].symbol, " ");
            terminal->term_frame[i].color = GCS_DEFAULT;
            terminal->term_frame[i].bg_color = GCS_BG_DEFAULT;
            strcpy(terminal->prev_frame[i].symbol, " ");
            terminal->prev_frame[i].color = GCS_DEFAULT;
            terminal->prev_frame[i].bg_color = GCS_BG_DEFAULT;
        }
        
    }
}
/**********************/

/**** IMPLEMENTED *****/
def_method(set_frame_width,int)
void set_frame_width(tframe_t *frame, int value){

    if(frame->con.max_w){
        if(value<frame->con.max_w && value >= frame->con.min_w){
            frame->dim.w = value;
        }else{
        if(value < frame->con.min_w){
            frame->dim.w = frame->con.min_w;
        }else{
            frame->dim.w = frame->con.max_w;
        }
        }
    }else{
        if(value >= frame->con.min_w){
            frame->dim.w = value;
        }else{
            frame->dim.w = frame->con.min_w;
        }
    }
}

def_method(set_frame_heigth,int)
void set_frame_heigth(tframe_t *frame, int value){

    if(value >= frame->con.min_h){
        frame->dim.h = value;
    }else{
        frame->dim.h = frame->con.min_h;
    }
    if(frame->con.max_h){
        if(value<frame->con.max_h){
            frame->dim.h = value;
        }else{
            frame->dim.h = frame->con.max_h;
        }
    }
}

def_method(set_frame_max_width,int)
void set_frame_max_width(tframe_t *frame, int value){
    frame->con.max_w = value;
}

def_method(set_frame_min_width,int)
void set_frame_min_width(tframe_t *frame, int value){
    if(value>=0){
        frame->con.min_w = value;
    }
}

def_method(set_frame_max_heigth,int)
void set_frame_max_heigth(tframe_t *frame, int value){
    frame->con.max_h = value;

}

def_method(set_frame_min_heigth,int)
void set_frame_min_heigth(tframe_t *frame, int value){
    if(value>=0){
        frame->con.min_h = value;
    }
}

int init_tframe(tframe_t *frame){
    if(!frame) return 1;

    frame->set.h = (void(*)(int))bind_method(frame, set_frame_heigth);
    frame->set.w = (void(*)(int))bind_method(frame, set_frame_width);
    frame->set.max_h = (void(*)(int))bind_method(frame, set_frame_max_heigth);
    frame->set.max_w = (void(*)(int))bind_method(frame, set_frame_max_width);
    frame->set.min_h = (void(*)(int))bind_method(frame, set_frame_min_heigth);
    frame->set.min_w = (void(*)(int))bind_method(frame, set_frame_min_width);
    frame->con.max_h = 0;
    frame->con.max_w = 0;
    frame->dim.r = 0;
    frame->dim.c = 0;
    frame->dim.w = 0;
    frame->dim.h = 0;
    frame->dim.tile.bg_color = GCS_BG_DEFAULT;
    frame->dim.tile.bg_color = GCS_DEFAULT;
    strcpy(frame->dim.tile.symbol,"X");

    return 0;
}

/***** DEPRICATED *****/
void force_present(term_w_t *terminal){

    if(!terminal->cursor_data.hidden) hide_cursor();
    cursor_origin();

    for(int i=0; i<terminal->nrows*terminal->ncols; i++){
            printf("\x1b[%d;%dH", i/terminal->ncols+1, i%terminal->ncols+1);
            printf("%s%s%s\x1b[0m",terminal->term_frame[i].bg_color, terminal->term_frame[i].color, terminal->term_frame[i].symbol);
    }

    printf("\x1b[%d;%dH", terminal->cursor_data.row+1, terminal->cursor_data.col+1);
    if(!terminal->cursor_data.hidden) show_cursor();

    fflush(stdout);
    memcpy(terminal->prev_frame, terminal->term_frame, sizeof(tile_t)*terminal->ncols*terminal->nrows);
}
/**********************/

/**** IMPLEMENTED *****/
int init_tui(term_w_t *terminal){
    if(!terminal) return 1;

    struct winsize ws;
    ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws);
    terminal->nrows = ws.ws_row;
    terminal->ncols = ws.ws_col;

    terminal->term_frame = malloc(sizeof(tile_t)*terminal->nrows*terminal->ncols);
    if(!terminal->term_frame) return 1;

    terminal->prev_frame = malloc(sizeof(tile_t)*terminal->nrows*terminal->ncols);
    if(!terminal->prev_frame) return 1;

    if(!original_term){
        original_term = terminal;
    }


    int i,j;
    for(i=0;i<terminal->nrows*terminal->ncols;i++){
        strcpy(terminal->term_frame[i].symbol, " ");
        terminal->term_frame[i].color = GCS_DEFAULT;
        terminal->term_frame[i].bg_color = GCS_BG_DEFAULT;
        strcpy(terminal->prev_frame[i].symbol, " ");
        terminal->prev_frame[i].color = GCS_DEFAULT;
        terminal->prev_frame[i].bg_color = GCS_BG_DEFAULT;
    }

    terminal->clear = bind_action(terminal, clear_screen);
    terminal->present = bind_action(terminal, present);
    terminal->frame_resize = bind_action(terminal, frame_resize);

    terminal->io_block = (int(*)(int))bind_method(terminal, io_blocking_handler);
    terminal->cursor = (void(*)(int))bind_method(terminal, cursor_handler);
    terminal->horz_strdisp = (int(*)(str_t))bind_method(terminal, horz_strdisp);
    terminal->vert_strdisp = (int(*)(str_t))bind_method(terminal, vert_strdisp);
    terminal->horz_tiledisp = (int(*)(fstr_t))bind_method(terminal, horz_tiledisp);
    terminal->vert_tiledisp = (int(*)(fstr_t))bind_method(terminal, vert_tiledisp);
    terminal->draw_rect = (void(*)(rect_t))bind_method(terminal, draw_rect);
    terminal->draw_border = (void(*)(rect_t))bind_method(terminal, draw_border);
    terminal->draw_frame = (void(*)(rect_t))bind_method(terminal, draw_frame);


    cursor_origin();
    terminal->cursor_data.row = 0;
    terminal->cursor_data.col = 0;

    force_present(terminal);

    if(tcgetattr(STDIN_FILENO, &terminal->old_conf) == -1) return -1;

    struct sigaction sa = {0};
    sa.sa_handler = sigint_handler;
    sa.sa_flags   = SA_RESTART;
    sigaction(SIGINT, &sa, NULL);
    sa.sa_sigaction = segv_handler;
    sa.sa_flags = SA_SIGINFO | SA_RESTART;
    sigaction(SIGSEGV, &sa, NULL);

    return 0;
}
/**********************/